...
def get_dataset_class(name, load_mode):
    ...
    elif name == "humanml" or name == "humanml_266" or name == "humanml_mask" or name == "humanml_smplx":
...
def get_collate_fn(name, load_mode='train'):
    ...
    if name in ["humanml", "humanml_266", "humanml_mask", "humanml_smplx", "kit"]:
        return t2m_collate
    elif name == 'pw3d':
        return pw3d_collate
    elif name == 'babel':
...
def get_dataset(name, num_frames, split='train', load_mode='train', batch_size=None, opt=None, short_db=False, cropping_sampler=False, size=None):
    ...
    elif name in ["humanml_266"]:
        dataset = DATA(split=split, datapath='./dataset/humanml_266_opt.txt', num_frames=num_frames, load_mode=load_mode, size=size)
    elif name in ["humanml_mask"]:
        dataset = DATA(split=split, datapath='./dataset/humanml_mask_opt.txt', num_frames=num_frames, load_mode=load_mode, size=size)
    elif name in ["humanml_smplx"]:
        dataset = DATA(split=split, datapath='./dataset/humanml_smplx_opt.txt', num_frames=num_frames, load_mode=load_mode, size=size)
    elif name == "babel":
...
def get_dataset_loader(name, batch_size, num_frames, split='train', load_mode='train', opt=None, short_db=False, cropping_sampler=False, size=None):
    if load_mode == 'text_only':
        load_mode = 'train'
    # shuffle = False if load_mode == 'gt' or load_mode == 'eval' else True
    shuffle = True
