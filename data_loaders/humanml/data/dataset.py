...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer, num_frames, size=None, **kwargs):
        ...
        mean = mean[:opt.dim_pose]
        std = std[:opt.dim_pose]
        ...
        for name in tqdm(id_list):
            try:
                ...
                # if not os.path.exists(motion_path) or os.path.basename(motion_path)[0] == 'M':
                if not os.path.exists(motion_path):
                    skip_1 += 1
                    continue

                motion = np.load(motion_path)
                lens.append(len(motion))
                if (len(motion)) < min_motion_len or (len(motion) >= 200):
                    skip_2 += 1
                    continue
                ...
                # w/o hand
                # hand = motion[:, 69:]
                motion = motion[:, :opt.dim_pose]

                # w/o velocity (for debug)
                motion[:, 7+21*3+21*6 : 7+21*3+21*6+22*3] = 0
            ...
            # if len(data_dict.keys()) > 256:
            # # if len(data_dict.keys()) > 32:
            #     break

            if size is not None:
                if len(data_dict.keys()) > size:
                    break
        
        print("{} data collected.".format(len(data_dict.keys())))

        name_list, length_list = zip(*sorted(zip(new_name_list, length_list), key=lambda x: x[1]))

        # # 単語の検索
        # ts = np.array([[t['caption'] for t in data_dict[k]['text']] for k in data_dict.keys()])
        # kw = 'wiping a table'
        # ind = [i for i, text in enumerate(ts) for t in text if kw in t]
        # print([t_ for t in ts[ind] for t_ in t if kw in t_])
...
class HumanML3D(data.Dataset):
    def __init__(self, load_mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        if 'smplx' in datapath:
            self.dataset_name = 't2m_smplx'
            self.dataname = 't2m_smplx'
        elif '266' in datapath:
            self.dataset_name = 't2m_266'
            self.dataname = 't2m_266'        
        elif 'mask' in datapath:
            self.dataset_name = 't2m_mask'
            self.dataname = 't2m_mask'        
        else:
        ...
        if opt.dataset_name == "t2m_266" or opt.dataset_name == "t2m_mask":
            self.mean = np.load(pjoin(opt.data_root, 'Mean_266.npy'))
            self.std = np.load(pjoin(opt.data_root, 'Std_266.npy'))
        elif load_mode == 'gt':
            ...
            elif opt.dataset_name == "t2m_266":
                self.mean = np.load(pjoin(opt.data_root, 'Mean_266.npy'))
                self.std = np.load(pjoin(opt.data_root, 'Std_266.npy'))
            elif opt.dataset_name == "t2m_smplx":
                self.mean = np.load(pjoin(opt.data_root, 'Mean_smplx.npy'))
                self.std = np.load(pjoin(opt.data_root, 'Std_smplx.npy'))
            else:
                raise FileNotFoundError()

        if load_mode == 'eval':
            if opt.dataset_name == "t2m_266" or opt.dataset_name == "t2m_mask":
                self.mean_for_eval = np.load(pjoin(opt.data_root, 'Mean_266.npy'))
                self.std_for_eval = np.load(pjoin(opt.data_root, 'Std_266.npy'))
            # used by T2M models (including evaluators)
