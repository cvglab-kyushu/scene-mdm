...
class CompMDMGeneratedDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        keyframes = [] if not 'keyframes' in data.keys() else data['keyframes']
        ...
        if self.dataset.load_mode == 'eval' and self.dataset.dataset_name != 'babel':  # re-norm is not needed for babel dataset
            ...
            gt_normed_motion = gt_motion
            if normed_motion.shape[-1] == 267:
                normed_motion = normed_motion[:, :-1]
                gt_normed_motion = gt_normed_motion[:, :-1]
            denormed_motion = self.dataset.t2m_dataset.inv_transform(normed_motion)
            gt_denormed_motion = self.dataset.t2m_dataset.inv_transform(gt_normed_motion)
        ...
        return word_embeddings, pos_one_hots, caption, sent_len, motion, gt_motion, m_length, '_'.join(tokens), keyframes
        # return word_embeddings, pos_one_hots, caption, sent_len, motion, gt_motion, m_length, '_'.join(tokens)
    ...
    def __init__(self, args, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1.):
        ...
        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                ...
                keyframes = []
                if args.dataset in ["humanml_mask"]:
                    mask, keyframes = get_inpainting_mask(args.inpainting_mask, motion.shape, model_kwargs['y']['lengths'], keyframes=None)
                    ...
                    motion = torch.cat([motion, model_kwargs['y']['key_mask']], axis=1)
                    model_kwargs['y']['inpainted_motion'] = motion

                else:
                    mask, keyframes = get_inpainting_mask(args.inpainting_mask, motion.shape, model_kwargs['y']['lengths'], keyframes=None)
                    model_kwargs['y']['inpainting_mask'] = torch.tensor(mask).float().to(dist_util.dev())

                keyframes = [[int(k) for k in kf] for kf in keyframes]
                ...
                for t in range(repeat_times):
                    ...
                    if t == 0:
                        sub_dicts = [{'gt_motion': motion[bs_i].squeeze().permute(1,0).cpu().numpy(),
                                    'motion': sample[bs_i].squeeze().permute(1,0).cpu().numpy(),
                                    'length': int(model_kwargs['y']['lengths'][bs_i].cpu().numpy()),
                                    'caption': model_kwargs['y']['text'][bs_i],
                                    'tokens': tokens[bs_i],
                                    'cap_len': len(tokens[bs_i]),
                                    'is_transition': model_kwargs['y']['is_transition'][bs_i],
                                    'keyframes': keyframes[bs_i],
                                    } for bs_i in range(dataloader.batch_size)]
                        generated_motion += sub_dicts
...
class HumaniseDataset(Dataset):

    def __init__(self, args, humanise_output_dir, dataloader):
        generated_motion = []
        self.dataset = dataloader.dataset
        ...
        for npy in glob.glob(os.path.join(humanise_output_dir, "*.npy")):
            motion = np.load(npy, allow_pickle=True).item()['data']
            ...
            word_list, pose_list = self.process_text(caption)
            tokens = ' '.join(['%s/%s'%(word_list[i], pose_list[i]) for i in range(len(word_list))])
            tokens = tokens.split(' ')


            sub_dicts = [{'motion': motion,
                        'length': len(motion),
                        'caption': caption,
                        'tokens': tokens,
                        'cap_len': len(tokens)}]
    ...
    def __getitem__(self, item):
        ...
        # renormed_motion = (denormed_motion - self.dataset.mean) / self.dataset.std  # according to T2M norms
        renormed_motion = (denormed_motion - self.dataset.mean_for_eval) / self.dataset.std_for_eval  # according to T2M norms
        motion = renormed_motion
        # This step is needed because T2M evaluators expect their norm convention
