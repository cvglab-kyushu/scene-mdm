...
if __name__ == '__main__':
    ...
    if opt.dataset_name == 't2m':
        opt.data_root = '../motion-diffusion-model/dataset/HumanML3D'
    ...
    elif opt.dataset_name == 't2m_66':
        opt.data_root = '../motion-diffusion-model/dataset/HumanML3D'
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs_66')
        opt.text_dir = pjoin(opt.data_root, 'texts')
        opt.joints_num = 22
        opt.max_motion_length = 196
        opt.dim_pose = 66
    ...
    if opt.dataset_name == 'babel':
        ...
        if opt.dataset_name == 't2m_66':
            mean = np.load(pjoin(opt.data_root, 'Mean_66.npy'))
            std = np.load(pjoin(opt.data_root, 'Std_66.npy'))
        else:
        ...
        train_args['num_frames'] = 196 if 't2m' in opt.dataset else False
        val_args['num_frames'] = 196 if 't2m' in opt.dataset else False
