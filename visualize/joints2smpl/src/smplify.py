...
def guess_init_3d(model_joints, 
    ...
    elif joints_category=="AMASS" or joints_category=="AMASS_smplx":
...
class SMPLify3D():
    ...
    def __init__(self,
        ...
        self.pose_prior = MaxMixturePrior(prior_folder=config.GMM_MODEL_DIR,
                                          num_gaussians=8,
                                          dtype=torch.float32,
                                          joint_category=joints_category).to(device)
        # collision part
        ...
        elif joints_category=="AMASS_smplx":
            self.smpl_index = config.amass_smplx_idx
            self.corr_index = config.amass_smplx_idx
        else:
    ...
    def infer(self, init_pose, init_betas):
        # Split SMPL pose to body pose and global orientation
        body_pose = init_pose[:, 3:].detach().clone()
        global_orient = init_pose[:, :3].detach().clone()
        betas = init_betas.detach().clone()
