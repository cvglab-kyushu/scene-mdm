...
from src.cmd_parser import parse_config, body_sampling_index, furniture_data
...
from src.motion_process import convert_humanml_to_266
...
if __name__ == '__main__':
    ...
    args_yaml = "./cfg_files/affordance_args.yaml"
    mdm_output_dir = "../save/humanml_only_text_condition/result_a_person_sits_on_a_sofa_and_waves_goodbye!/sample25_rep00_iter=20"
    output_ply_dir  = mdm_output_dir + "_converted"
    show_gen_sample = False
    ...
    # specified_object = "bed"
    # specified_object = "table"
    specified_object = "sofa"
    # specified_object = "chair"
    # specified_object = "floor"
    # specified_object = "sink"
    # specified_object = "wall"
    # specified_object = "shelving"
    # specified_object = ""
    ...
    neutral_bm = BodyModel(bm_fname=neutral_bm_path, num_betas=10, num_dmpls=8, dmpl_fname=neutral_dmpl_path)
    ...
    args.specified_object = specified_object
    args_dict['scene_name'] = scene_name
    args_dict['specified_object'] = specified_object
    ...
    args.show_gen_sample = show_gen_sample
    ...
    neutral_bm = neutral_bm.to(device)
    ...
    for frame in tqdm(range(0, motion_length, 3)):
        ...
        if args.use_semantics:
            ...
            for i in range(gen_batches.shape[0]):
                ...
                if args_dict['specified_object'] != "floor":
                    not_void_and_floor = x_semantics[x_semantics != 0]
                else:
    ...
    if len(losses > 0):
        ...
        for i in range(min(args.num_rendered_samples, len(losses))):
            ...
            convert_smplx2humanml(neutral_bm, device, save_pkl_path, save_pkl_path[:-4] + ".npy", scene_name=scene_name)
            ...
            if args.render or args.save_meshes:
                ...
                rgb_colors = np.array([ImageColor.getcolor(hex, "RGB") 
                    for hex in [furniture_data[furniture_data['mpcat40index']==label]['hex'].iloc[0] for label in targets]])
                targets = gen_batch[:, :, 1:].argmax(dim=-1).type(torch.long).reshape(batch_size, -1)[0].detach().cpu().numpy()
                ...
                # Convert original motion to POSA output space
                misc_utils.convert_HumanML_to_POSA(mdm_output_dir, 
                                               output_ply_dir, 
                                               keyframes=keyframes, 
                                               R=param['R_fm_orig'], 
                                               t=param['t_fm_orig'],
