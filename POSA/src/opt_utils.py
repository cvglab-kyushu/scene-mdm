...
def compute_afford_loss(vertices=None, scene_data=None, gen_batch=None, pen_w=0.0, no_obj_classes=None,
                        use_semantics=False,
                        semantics_w=0.0, specified_object='', **kwargs):
    ...
    if use_semantics:
        ...
        if specified_object != '':
            targets = replace_object(targets, specified_object)
...
def replace_object(targets, specified_object):
    replaced_targets = copy.deepcopy(targets)
    convert_objects = { 'bed': [11, [2, 3, 5, 10, 34]],
                        'table': [5, [3, 10, 11, 34]],
                        'chair': [3, [8, 10, 11, 18, 19, 34]], 
                        'sofa': [10, [3, 8, 11, 18, 19, 34]], 
                        'wall': [1, [3, 4, 5, 6, 9, 10, 11, 22, 33, 35, 36]], 
                        'floor': [2, [1, 3, 4, 5, 9, 10, 11, 12, 16, ]],
                        'shelving': [31, [1, 4, 5, 6, 7, 9, 12, 13, 15, 17, 21, 22, 26, 27, 28, 32, 36, 39]],
                        'sink': [15, [4, 5, 7, 9, 10, 12, 18, 19, 21, 22, 24, 26, 31, 32, 33, 35, 36, 38, 39]]} 
    for i in range(targets.shape[0]):
        for obj_id in convert_objects[specified_object][1]:
            replaced_targets[i, targets[i] == obj_id] = convert_objects[specified_object][0]
            
        # tableにしたときにfloorをvoidに
        if specified_object == 'table':
            replaced_targets[i, targets[i] == 2] = 0
    return replaced_targets


def eval_init_points(init_pos=None, init_ang=None, vertices=None, scene_data=None, gen_batch=None, specified_object='', init_semantics_w=1.0, **kwargs):
    del kwargs['semantics_w']
    with torch.no_grad():
        losses = []
        ...
        for i in tqdm(range(len(init_pos_batches))):
            ...
            contact_loss, pen_loss, semantics_loss = compute_afford_loss(vertices=curr_vertices, scene_data=scene_data,
                                                                        gen_batch=gen_batch, specified_object=specified_object,
                                                                        semantics_w=init_semantics_w, **kwargs)
...
def init_points_culling(init_pos=None, vertices=None, scene_data=None, gen_batch=None, specified_object='', max_init_points=50, **kwargs):
    init_ang = []
    ...
    losses, init_pos, init_ang = eval_init_points(init_pos=init_pos, init_ang=init_ang,
                                                  ...
                                                  gen_batch=gen_batch,
                                                  specified_object=specified_object, **kwargs)
...
class opt_wrapper(object):
    def __init__(self, vertices=None, vertices_can=None, pelvis=None, scene_data=None,
                 ...
                 semantics_w=None, init_body_pose=None, pose_w=None, specified_object='', **kwargs):
    ...
    def compute_loss(self, t_free, y_ang):
        ...
        contact_loss, pen_loss, semantic_loss = compute_afford_loss(vertices=vertices_local, scene_data=self.scene_data,
                                                                    ...
                                                                    specified_object=self.specified_object)
