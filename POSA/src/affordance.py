...
if __name__ == '__main__':
    ...
    affordance_dir = osp.join(args_dict.get('save_dir'))
    ...
    for pkl_file_path in pkl_file_paths:
        ...
        # tmp_dir = os.path.join(affordance_dir, "tmp")
        # os.makedirs(tmp_dir, exist_ok=True)
        # import joblib
        # mesh = trimesh.Trimesh(vertices=joblib.load(pkl_file_path)['vertices'], faces=faces_arr)
        # mesh.export(os.path.join(tmp_dir, "body_input.ply"))
        # mesh = trimesh.Trimesh(vertices=vertices_org.detach().cpu().numpy(), faces=faces_arr)
        # mesh.export(os.path.join(tmp_dir, "body_org.ply"))
        # mesh = trimesh.Trimesh(vertices=vertices_can.detach().cpu().numpy(), faces=faces_arr)
        # mesh.export(os.path.join(tmp_dir, "body_can.ply"))
        ...
        for sample_id in range(gen_batches.shape[0]):
            ...
            if len(losses > 0):
                ...
                for i in range(min(args.num_rendered_samples, len(losses))):
                    ...
                    param['R_fm_orig'] = rot_mat
                    param['aa_fm_orig'] = cv2.Rodrigues(rot_mat.detach().cpu().squeeze().numpy())[0]
                    # param['t_fm_orig'] = torch_param['transl'] - t_before
                    param['t_fm_orig'] = t_free
                    ...
                    if args.render or args.save_meshes:
                        ...
                        if args_dict['specified_object'] != '':
                            ...
                            color_body = trimesh.Trimesh(results[ind][body_sampling_index], faces_arr2, vertex_colors=rgb_colors, process=False)
                            color_body.export(osp.join(affordance_dir, 'meshes_color', args_dict.get('scene_name'),
                                                 '{}_{}_specified={}.obj'.format(result_filename, i, args_dict['specified_object'])))
