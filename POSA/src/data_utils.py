...
def pkl_to_canonical(pkl_file_path, device, dtype, batch_size, model_folder=None, vertices_clothed=None, param=None, **kwargs):
    ...
    if pkl_file_path is not None:
        param = joblib.load(pkl_file_path)
    num_pca_comps = 6
    ...
    # vertices[:, [0,2]] -= pelvis[:, [0,2]]
    # vertices[:, [0,1]] -= pelvis[:, [0,1]]
    # vertices[:, [1,2]] -= pelvis[:, [1,2]]
    # vertices = torch.matmul(R_smpl2scene, vertices.T).t()
...
def pklmdm_to_canonical(pkl_file_path, device, dtype, batch_size, model_folder=None, vertices_clothed=None, param=None, **kwargs):
    R_can = torch.tensor(eulerangles.euler2mat(np.pi, np.pi, 0, 'syzx'), dtype=dtype, device=device)
    R_smpl2scene = torch.tensor(eulerangles.euler2mat(np.pi / 2, 0, 0, 'sxyz'), dtype=dtype, device=device)
    # with open(pkl_file_path, 'rb') as f:
    #     param = pickle.load(f)
    ...
    torch_param['left_hand_pose'] = torch_param['left_hand_pose'][:, :num_pca_comps]
    torch_param['right_hand_pose'] = torch_param['right_hand_pose'][:, :num_pca_comps]
    torch_param['transl'] = torch_param['transl'].unsqueeze(0)

    # y座標(高さ)はそのままで、x,zはCanonical座標に
    torch_param['transl'][:, [0,2]] -= torch_param['transl'][:, [0,2]]

    # x軸は回転せず、y軸とz軸だけCanonicalになるように回転
    r_canonical = compute_canonical_transform(torch.tensor(param['global_orient'], dtype=dtype, device=device).unsqueeze(0))
    aa_canonical = cv2.Rodrigues(r_canonical.detach().cpu().numpy())[0].T[0]
    torch_param['global_orient'] = torch.tensor(aa_canonical, dtype=dtype, device=device)
    ...
    vertices_can = torch.matmul(R_can, (vertices - pelvis).t()).t()
    vertices_org = torch.matmul(R_smpl2scene, (vertices - pelvis).t()).t()
    ...
    return vertices_org, vertices_can, body_model.faces, body_model, R_can, pelvis, torch_param, vertices_clothed