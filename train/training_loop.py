...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        self.log_path = self.save_dir + "/log.xlsx"
        if not os.path.exists(self.log_path):
            log = {'epoch':[], 'loss': []}
            pd.DataFrame(log).to_excel(self.log_path, index=False)
    ...
    def _load_optimizer_state(self):
        ...
        opt_checkpoint = bf.join(
            bf.dirname(main_checkpoint), f"opt.pt"
        )
    ...
    def run_loop(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for motion, cond in tqdm(self.data):
                ...
                if self.dataset == 'humanml_mask':
                    pad = torch.zeros([cond['y']['inpainting_mask'].shape[0], 1, 1, cond['y']['inpainting_mask'].shape[-1]])
                    cond['y']['inpainting_mask'] = torch.cat([cond['y']['inpainting_mask'], pad], dim=1)
                    motion = torch.cat([motion, cond['y']['key_mask']], axis=1)
                ...
                for key in losses.keys():
                    if key in each_loss.keys():
                        each_loss[key] += losses[key]
                    else:
                        each_loss[key] = losses[key]
            ...
            for key in each_loss.keys():
                each_loss[key] /= n_step
                
            if min_loss > each_loss["loss"]:
                self.save()
                min_loss = each_loss["loss"]

            print('Epoch[{}]: loss[{:0.5f}]'.format(epoch, each_loss["loss"]))

            log = pd.read_excel(self.log_path)
            n = len(log)
            log.at[n, 'epoch'] = epoch
            for key in each_loss.keys():
                log.at[n, key] = each_loss[key]
            pd.DataFrame(log).to_excel(self.log_path, index=False)
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            compute_losses = functools.partial(
                ...
                x_means=False,   # input mean motion
        ...
        return {key: losses[key].mean().detach().cpu().numpy() for key in losses.keys()}
