...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.replace_keypose = replace_keypose
        ...
        self.lambda_keysmooth = lambda_keysmooth
    ...
    def p_mean_variance(
        ...
        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            ...
            model_output = (model_output * (1 - inpainting_mask)) + (inpainted_motion * inpainting_mask) # Replace masked elements
    ...
    def training_losses(self, model, x_start, t, x_means=False, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        if x_means:
            mean_motion = torch.Tensor(dataset.mean).unsqueeze(0).unsqueeze(1).unsqueeze(2).repeat(
                model_kwargs['y']['mask'].shape[0], 1, model_kwargs['y']['mask'].shape[3], 1).permute(0, 3, 1, 2).to(device='cuda:0')
            mean_motion *= (1. - model_kwargs['y']['inpainting_mask'])
            input_motions = x_start * model_kwargs['y']['inpainting_mask'] + mean_motion
            input_motions = input_motions.to(dtype=torch.float32)
            x_t = self.q_sample(input_motions, t, noise=noise, model_kwargs=model_kwargs)
        else:
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.replace_keypose:
                key_mask = model_kwargs['y']['key_mask'].type(torch.int)
                model_output = (1 - key_mask) * model_output + key_mask * x_start   # replace keyposes to GT
            ...
            assert torch.allclose(x_start, target) # ModelMeanType.START_X
            ...
            if self.lambda_keysmooth > 0.:
                target_vel = (target[..., 1:] - target[..., :-1])
                model_output_vel = (model_output[..., 1:] - model_output[..., :-1])
                terms["keysmooth"] = self.masked_l2(target_vel, model_output_vel, model_kwargs['y']['key_mask_vel'])  # mean_flat((target_xyz - model_output_xyz) ** 2)
            ...
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            (self.lambda_keysmooth * terms.get('keysmooth', 0.)) +\
